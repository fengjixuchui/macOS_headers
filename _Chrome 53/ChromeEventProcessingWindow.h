//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Jun  3 2014 10:55:11).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import <Google Chrome Framework/UnderlayOpenGLHostingWindow.h>

#import "CommandDispatchingWindow.h"

@interface ChromeEventProcessingWindow : UnderlayOpenGLHostingWindow <CommandDispatchingWindow>
{
    struct scoped_nsobject<CommandDispatcher> commandDispatcher_;
    struct scoped_nsobject<ChromeCommandDispatcherDelegate> commandDispatcherDelegate_;
    struct scoped_nsprotocol<id<UserInterfaceItemCommandHandler>> commandHandler_;
}

- (id).cxx_construct;
- (void).cxx_destruct;
- (BOOL)validateUserInterfaceItem:(id)arg1;
- (void)sendEvent:(id)arg1;
- (BOOL)performKeyEquivalent:(id)arg1;
- (void)commandDispatchUsingKeyModifiers:(id)arg1;
- (void)commandDispatch:(id)arg1;
- (BOOL)defaultPerformKeyEquivalent:(id)arg1;
- (BOOL)redispatchKeyEvent:(id)arg1;
- (void)setCommandHandler:(id)arg1;
- (BOOL)handleExtraKeyboardShortcut:(id)arg1;
- (id)initWithContentRect:(struct CGRect)arg1 styleMask:(unsigned long long)arg2 backing:(unsigned long long)arg3 defer:(BOOL)arg4;

@end

